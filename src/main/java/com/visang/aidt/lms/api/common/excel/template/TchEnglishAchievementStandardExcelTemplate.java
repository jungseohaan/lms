...
@Slf4j
public class TchEnglishAchievementStandardExcelTemplate implements ExcelTemplateCallback {

    @Override
    public ExcelTemplateType getTemplateType() {
        return ExcelTemplateType.TCH_ENGLISH_ACHIEVEMENT_STANDARD;
    }

    @Override
    public void buildExcelWorkbook(Map<String, Object> model, Collection<?> data, Workbook workbook) {
        log.info("(교사) 영어 성취기준 Excel 템플릿 생성 시작");
        
        Sheet sheet = workbook.createSheet(ExcelTemplateType.TCH_ENGLISH_ACHIEVEMENT_STANDARD.getDescription());
        ...
        String[] headers = {"내용 체계 영역", "성취기준 코드", "1단계 내용 요소", "2단계 내용 요소", "우리 반 성취도"};
        
        for (int i = 0; i < headers.length; i++) {
            Cell cell = headerRow.createCell(i);
            cell.setCellValue(headers[i]);
            if (i == 4) {
                cell.setCellStyle(headerStyleForAchievement); // 성취도 컬럼만 짙은 남색으로 설정
            } else {
        ...
        sheet.setColumnWidth(4, 20 * 256); // 우리 반 성취도
